...
class AlarmScreenActivity : AppCompatActivity() {
    ...
    companion object {
        ...
        const val EXTRA_ALLOW_SIMULTANEOUS_PLAYBACK = "allow_simultaneous_playback"
    }
    ...
    private var allowSimultaneousPlayback: Boolean = false // New setting
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            playAlarmSound()
            // Start bell animation after a short delay to ensure UI is fully rendered
            handler.postDelayed({
                startAnimatedBell()
            }, 100)
            startTimeUpdates()
            
            Log.d("AlarmScreenActivity", "onCreate completed successfully")
        } catch (e: Exception) {
            Log.e("AlarmScreenActivity", "Error during onCreate initialization", e)
            Toast.makeText(this, "Error initializing alarm screen: ${e.message}", Toast.LENGTH_LONG).show()
            finish()
        }
    }
    ...
    private fun getIntentExtras() {
        ...
        ttsVoice = intent.getStringExtra(EXTRA_TTS_VOICE) ?: "female"
        hasVibration = intent.getBooleanExtra(EXTRA_HAS_VIBRATION, true)
        // Get the simultaneous playback setting
        allowSimultaneousPlayback = intent.getBooleanExtra(EXTRA_ALLOW_SIMULTANEOUS_PLAYBACK, false)
        
        Log.d("AlarmScreenActivity", "Alarm screen started - Title: $alarmTitle, Ringtone: $ringtoneName, URI: $ringtoneUri, Volume: $alarmVolume, Simultaneous: $allowSimultaneousPlayback")
    }
    ...
    private fun setupUI() {
        ...
        // Ensure bell animation is started
        startAnimatedBell()
    }
        ...
        try {
            ...
            audioSequenceManager?.let { manager ->
                manager.initialize(
                    ...
                    // Use the same volume for all audio types
                    ...
                    allowSimultaneousPlayback = allowSimultaneousPlayback
                )
    ...
    private fun startRingtonePlayback() {
        try {
            mediaPlayer = MediaPlayer().apply {
                ...
                // Configure looping and USE USER-SELECTED VOLUME instead of maximum
                isLooping = true
                
                // Use user-selected volume instead of forcing maximum
    ...
    private fun startAnimatedBell() {
        try {
            ...
            if (drawable is android.graphics.drawable.AnimatedVectorDrawable) {
                // Ensure the drawable is visible and start animation
                imageAlarmBell.visibility = View.VISIBLE
                drawable.start()
                Log.d("AlarmScreenActivity", "Animated bell started successfully")
            } else {
                Log.d("AlarmScreenActivity", "Drawable is not AnimatedVectorDrawable, using fallback animation")
    ...
    private fun startBellAnimation() {
        try {
            ...
            Log.e("AlarmScreenActivity", "Error starting fallback bell animation", e)
        }
    }
        ...
        try {
            ...
            mediaPlayer = MediaPlayer().apply {
                try {
                    ...
                    prepare()
                    start()
                    Log.d("AlarmScreenActivity", "Fallback notification sound started successfully with volume: $alarmVolume")
                } catch (e: Exception) {
                    ...
                    try {
                        ...
                        prepare()
                        start()
                        Log.d("AlarmScreenActivity", "Fallback ringtone sound started with volume: $alarmVolume")
                    } catch (e2: Exception) {
