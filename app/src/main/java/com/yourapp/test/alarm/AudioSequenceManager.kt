...
class AudioSequenceManager(private val context: Context) {
    ...
    fun initialize(
        ...
        allowSimultaneousPlayback: Boolean = false // New parameter
    ) {
        ...
        this.allowSimultaneousPlayback = allowSimultaneousPlayback // Set the new parameter
        
        Log.d(TAG, "Audio sequence initialized - Voice: $hasVoiceOverlay, TTS: $hasTtsOverlay, Volume: $ringtoneVolume, Simultaneous: $allowSimultaneousPlayback")
    ...
    fun startSequence() {
        ...
        // If simultaneous playback is allowed and both TTS and voice overlay are enabled, play them together
        if (allowSimultaneousPlayback && hasTtsOverlay && hasVoiceOverlay && 
            !ttsText.isNullOrBlank() && !voiceRecordingPath.isNullOrEmpty()) {
            Log.d(TAG, "🔄 Starting simultaneous TTS and voice overlay")
            startSimultaneousPlayback()
        }
        // If TTS overlay is enabled, start TTS loop directly
        else if (hasTtsOverlay && !ttsText.isNullOrBlank()) {
    ...
    /**
     * Play TTS and voice recording simultaneously
     */
    private fun startSimultaneousPlayback() {
        if (!isPlaying) return
        
        Log.d(TAG, "🔄 Starting simultaneous TTS and voice recording playback")
        
        try {
            // Start ringtone with the same volume as other audio types
            ringtonePlayer = MediaPlayer().apply {
                ...
                isLooping = true
                // Use the same volume for ringtone as other audio types
            ...
            Log.d(TAG, "🔔 Ringtone started with volume: $ringtoneVolume")
            
            // Start voice overlay with the same volume as other audio types
            voicePlayer = MediaPlayer().apply {
                ...
                setOnPreparedListener {
                    Log.d(TAG, "✅ Voice playback prepared")
                }
            ...
            Log.e(TAG, "Error starting simultaneous playback", e)
    ...
    private fun onAlarmPhaseComplete() {
        ...
        // Move to next phase: TTS or voice overlay, or repeat alarm
        if (hasTtsOverlay && !ttsText.isNullOrBlank()) {
            // Start TTS phase
            startTtsPhase()
        } else if (hasVoiceOverlay && !voiceRecordingPath.isNullOrEmpty()) {
